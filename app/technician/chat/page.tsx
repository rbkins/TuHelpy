...
export default function Chat() {
  ...
  const contacts = [
    { id: 1, name: "Juan Pérez", role: "Cliente", status: "En línea", avatar: "/placeholder-user.jpg" },
    { id: 2, name: "María Rodríguez", role: "Cliente", status: "Hace 5 min", avatar: "/placeholder-user.jpg" },
    { id: 3, name: "Carlos Sánchez", role: "Cliente", status: "Hace 1 hora", avatar: "/placeholder-user.jpg" },
  ]
    ...
    <div className="flex flex-col md:flex-row h-[calc(100vh-12rem)] md:h-[calc(100vh-14rem)]">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === "tech" ? "justify-end" : "justify-start"}`}>
              {msg.sender === "user" && (
              ...
              <div
                className={`rounded-lg p-3 max-w-[80%] ${
                  msg.sender === "tech" ? "bg-[#4A4A4A] text-white" : "bg-gray-100"
                }`}
              >
                <p className="text-sm">{msg.text}</p>
                <p className={`text-xs mt-1 ${msg.sender === "tech" ? "text-white/70" : "text-gray-500"}`}>
                  {msg.time}
                </p>
              </div>
              {msg.sender === "tech" && (
                <Avatar className="h-8 w-8 ml-2 mt-1">
                  <AvatarImage src="/placeholder-user.jpg" alt="Técnico" />
        ...
        <div className="p-4 border-t">
          <div className="flex items-center">
            ...
            />
            <Button className="ml-2 bg-[#4A4A4A] hover:bg-gray-600" onClick={handleSend} disabled={!message.trim()}>
